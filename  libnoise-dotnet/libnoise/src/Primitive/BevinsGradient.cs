...
namespace Graphics.Tools.Noise.Primitive {
	...
	public class BevinsGradient :PrimitiveModule, IModule3D {
		...
		protected static double[] _randomVectors =  {
		...
		public double GradientCoherentNoise3D(double x, double y, double z, long seed, NoiseQuality quality) {
			...
			switch(quality) {
				...
				case NoiseQuality.Standard:
					xs = Libnoise.SCurve3(x - (double)x0);
					ys = Libnoise.SCurve3(y - (double)y0);
					zs = Libnoise.SCurve3(z - (double)z0);
					break;

				case NoiseQuality.Best:
					xs = Libnoise.SCurve5(x - (double)x0);
					ys = Libnoise.SCurve5(y - (double)y0);
					zs = Libnoise.SCurve5(z - (double)z0);
					break;
			}//end switch
		...
		protected double GradientNoise3D(double fx, double fy, double fz, int ix, int iy, int iz, long seed) {
			...
			double xvGradient = _randomVectors[(vectorIndex << 2)];
			double yvGradient = _randomVectors[(vectorIndex << 2) + 1];
			double zvGradient = _randomVectors[(vectorIndex << 2) + 2];
