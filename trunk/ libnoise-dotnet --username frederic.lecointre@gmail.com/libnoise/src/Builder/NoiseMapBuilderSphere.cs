...
namespace Graphics.Tools.Noise.Builder {
	...
	public class NoiseMapBuilderSphere :NoiseMapBuilder {
		...
		public NoiseMapBuilderSphere() {
			SetBounds(-90.0, 90.0, -180.0, 180.0); // degrees
		}//end NoiseMapBuilder
		...
		public override void Build(){
			...
			double xDelta = lonExtent / (double)_width ;
			double yDelta = latExtent / (double)_height;
			...
			for (int y = 0; y < _height; y++) {
				...
				for (int x = 0; x < _width; x++) {
					float curValue = (float)model.GetValue(curLat, curLon);
